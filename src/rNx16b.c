...
static inline void RansEncSymbolInit(RansEncSymbol* s, uint32_t start, uint32_t freq, uint32_t scale_bits)
{
  ...
  // FIXME: temporary
  s->scale_bits = (uint16_t)scale_bits;
...
unsigned int rans_compress_bound_32x16_NoSimd(unsigned int size, int order, int *tab) {
  ...
  if (tab) *tab = tabsz;
  return (int)(1.05*size + tabsz);
}

unsigned char *rans_compress_O0_32x16_NoSimd(unsigned char *in, unsigned int in_size,
  ...
  int m = 0, M = 0;
  unsigned int bound = rans_compress_bound_32x16_NoSimd(in_size, 0, NULL);
  int64_t z;
  ...
  *cp++ = 0;
  ...
  for (i = (in_size &~(NX - 1)); i > 0; i -= NX) {
    for (z = NX - 1; z >= 0; z -= 4) {
      ...
      if (NX % 8 == 0) {
        z -= 4;
        RansEncSymbol *s0_ = &syms[in[i - (NX - z + 0)]];
        RansEncSymbol *s1_ = &syms[in[i - (NX - z + 1)]];
        RansEncSymbol *s2_ = &syms[in[i - (NX - z + 2)]];
        RansEncSymbol *s3_ = &syms[in[i - (NX - z + 3)]];
        RansEncPutSymbol(&ransN[z - 0], &ptr, s0_);
        RansEncPutSymbol(&ransN[z - 1], &ptr, s1_);
        RansEncPutSymbol(&ransN[z - 2], &ptr, s2_);
        RansEncPutSymbol(&ransN[z - 3], &ptr, s3_);
      }
    }
    if (z < -1)
    {
  ...
  *out_size = (uint32_t)((out_end - ptr) + tab_size);
...
unsigned char *rans_uncompress_O0_32x16_NoSimd(unsigned char *in, unsigned int in_size,
  unsigned char *out, unsigned int *out_size) {
  (void)in_size;
  ...
  for (i = 0; i < out_end; i += NX) {
    for (z = 0; z < NX; z += 4) {
      ...
      out[i + z + 0] = (unsigned char)S[0];
      out[i + z + 1] = (unsigned char)S[1];
      out[i + z + 2] = (unsigned char)S[2];
      out[i + z + 3] = (unsigned char)S[3];
...
unsigned char *rans_compress_O1_32x16_NoSimd(unsigned char *in, unsigned int in_size,
  unsigned char *out, unsigned int *out_size) {
  unsigned char *cp, *out_end;
  unsigned int tab_size, rle_i, rle_j;
  RansEncSymbol syms[256][256];
  int bound = rans_compress_bound_32x16_NoSimd(in_size, 1, NULL), z;
  RansState ransN[NX];
...
unsigned char *rans_uncompress_O1_32x16_NoSimd(unsigned char *in, unsigned int in_size,
  unsigned char *out, unsigned int *out_size) {
  (void)in_size;
  ...
  do {
    ...
    do {
      ...
      for (y = 0; y < F; y++) {
        ssym[i][y + C] = (uint16_t)j;
        sfb[i][y + C].f = (uint16_t)F;
        sfb[i][y + C].b = (uint16_t)y;
  ...
  for (; iN[0] < isz4; ) {
    for (z = 0; z < NX; z += 4) {
      ...
      c[z + 0] = (uint8_t)ssym[c[z + 0]][m[0]];
      c[z + 1] = (uint8_t)ssym[c[z + 1]][m[1]];
      c[z + 2] = (uint8_t)ssym[c[z + 2]][m[2]];
      c[z + 3] = (uint8_t)ssym[c[z + 3]][m[3]];
  ...
  for (; iN[z] < out_sz; ) {
    ...
    unsigned char C = (unsigned char)ssym[c[z]][m];
    out[iN[z]++] = C;
...
*/
unsigned char *rans_compress_to_32x16_NoSimd(unsigned char *in, unsigned int in_size,
  unsigned char *out, unsigned int *out_size,
  int order) {
  return order
    ? rans_compress_O1_32x16_NoSimd(in, in_size, out, out_size)
    : rans_compress_O0_32x16_NoSimd(in, in_size, out, out_size);
}

unsigned char *rans_compress_32x16_NoSimd(unsigned char *in, unsigned int in_size,
  unsigned int *out_size, int order) {
  return rans_compress_to_32x16_NoSimd(in, in_size, NULL, out_size, order);
}

unsigned char *rans_uncompress_to_32x16_NoSimd(unsigned char *in, unsigned int in_size,
  unsigned char *out, unsigned int *out_size,
  int order) {
  return order
    ? rans_uncompress_O1_32x16_NoSimd(in, in_size, out, out_size)
    : rans_uncompress_O0_32x16_NoSimd(in, in_size, out, out_size);
}

unsigned char *rans_uncompress_32x16_NoSimd(unsigned char *in, unsigned int in_size,
  unsigned int *out_size, int order) {
  return rans_uncompress_to_32x16_NoSimd(in, in_size, NULL, out_size, order);
}
